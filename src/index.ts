...
async function fetchCache(env: Env): Promise<BackpressureCache> {
  return (
...
async function cacheRequest(env: Env): Promise<Response> {
  const cache = await fetchCache(env);
